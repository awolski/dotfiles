...
(let* ((package--builtins nil)
       (packages
        '(auto-compile         ; automatically compile Emacs libraries
          ...
          org-tree-slide       ; A presentation tool for org-mode based
...
#+BEGIN_SRC emacs-lisp
(setq-default tab-width 8                       ; Smaller tabs
...
[[https://github.com/karthink/gptel][gptel]] is awesome.
...
Let's try out [[https://github.com/takaxp/org-tree-slide][org-tree-slide]] for presentations.
...
(add-hook 'org-tree-slide-stop-hook 'my/org-tree-slide-end)
#+END_SRC
...
(setq ledger-post-amount-alignment-column 70
      ledger-reports '(("budget" "%(binary) -f %(ledger-file) bal ^assets:cash ^liabilities:credit ^assets:owed -X £ --strict --cleared")
                       ("reconcile" "%(binary) -f %(ledger-file) reg %(account) --strict")
                       ("unknown_expenses" "%(binary) -f %(ledger-file) reg Expenses:Unknown --strict")
                       ("wise_interest" "%(binary) -f %(ledger-file) reg @Wise\\ Interest --subtotal")
                       ("wise_aud_interest" "%(binary) -f %(ledger-file) reg @Wise\\ AUD\\ Interest --subtotal")
...
(setq org-directory "~/git/org"
      org-agenda-files (list "~/git/peerj" org-directory)
...
(setq org-capture-templates
      '(("t" "Task" entry (file org-default-notes-file)
        ...
        ("p" "PeerJ")
        ("pc" "Meeting" entry (file+headline "~/git/peerj/peerj.org" "Meetings")
         "* MEETING Call with %? :MEETING:CALL:\n%T\n%U" :clock-in t :clock-resume t :time-prompt t)
        ("ps" "Stand up" entry (file+headline "~/git/peerj/peerj.org" "Meetings")
         "* MEETING Stand up :MEETING:CALL:\n<ps%?\n%T\n%U" :clock-in t :clock-resume t :time-prompt t)
        ("pt" "Task" entry (file+headline "~/git/peerj/peerj.org" "Tasks")
         "* TODO %?\n%u\n%a\n" :clock-in t :clock-resume t)
        ))
#+end_src
...
Let's also take [[https://github.com/yjwen/org-reveal/][org-reveal]] for a spin.
